...
async function updateBadge(enabled) {
  try {
    ...
    await chrome.action.setTitle({ title: enabled ? "Absoluto Glamur · LIGADA" : "Absoluto Glamur · DESLIGADA" });
  } catch {}
}
function renderPower(enabled) {
  ...
  powerText.textContent = enabled ? "Ativa automaticamente até você desligar." : "Não responderá à loja até ser ligada novamente.";
...
powerButton.addEventListener("click", async () => {
  ...
  renderPower(next);
  show(next ? "Extensão ligada. A sincronização automática está ativa." : "Extensão desligada. Clique em Ligar quando quiser reativar.", next ? "success" : "info");
});
importButton.addEventListener("click", async () => {
  ...
  try {
    ...
    if (String(payload.sid) !== productId) throw new Error(`O código foi gerado para o AliExpress ID ${payload.sid}, mas a aba aberta é ${productId}.`);
    ...
    const response = await chrome.runtime.sendMessage({ type: "AG_IMPORT_ALIEXPRESS_REVIEWS", bridgeCode: code, tabId: tab.id, productId });
    if (!response?.ok) throw new Error(response?.error || "Não foi possível concluir a importação.");
    show(`${response.imported} avaliação(ões) importada(s).\n${response.withPhotos || 0} com foto(s). Total detectado: ${response.remoteTotal || response.imported}.`, "success");
  } catch (error) {
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === "local" && changes.agExtensionEnabled) renderPower(changes.agExtensionEnabled.newValue !== false);
});
